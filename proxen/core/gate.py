...
class HealthGuard:
    """Per-upstream health tracker with three derived states:

    - `healthy`: requests pass through.  Consecutive failures accumulate.
    - `failing`: upstream is skipped entirely.  After `cooldown`
      seconds the guard transitions to `probing`.
    - `probing`: requests are allowed through as a probe.  A success
      resets to `healthy`; a failure returns to `failing`.

    State is derived from `_consecutive_failures` and `_failed_at`
    rather than stored explicitly, no state-machine bugs to worry about.
    """
    ...
    def is_healthy(self) -> bool:
        """Return True if requests should be routed to this upstream."""
